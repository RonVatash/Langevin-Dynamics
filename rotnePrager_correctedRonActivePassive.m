...
% corrected by Ron 20221211 - absulot value for distances and factor of 3
% in the yy and xx matrix
% This function is exactly the same as the regular "rotne prager" function
% I started this function in order to do two different radii but yet didn't
% do it - 20231022 Ron Vatash
function DMat = rotnePrager_correctedRonActivePassive(particlesMat, Dslow, Dfast, cfg)
d = 2; % The dimension of the problem. 
Rslow = cfg.R(1);
R = cfg.R(1); % CONTINUE FROM HERE NEEDS TO MODIFIED THIS FUNCTION TO WORK WITH TWO DIFFERENT PARTICLES
% The code is NOT scalable to more or less dimensions, the variable d is
% only present for better readability.
D = Dslow; % 20231004 right now it works for one bead size only. need to understand how to correct it later
...
for checkedParticle = 1:numOfParticles
    for pairedParticle = 1:(checkedParticle-1)
        ...
        xDiff = abs(particlesMat(checkedParticle,1) - particlesMat(pairedParticle,1));
        yDiff = abs(particlesMat(checkedParticle,2) - particlesMat(pairedParticle,2));
